// auth.js - Complete Integrated Authentication System
const authSystem = {
  // Global variables
  verificationCode: "",
  pendingUser: null,
  ...
  bindEvents: function () {
    ...
    // Prevent dropdown from closing when clicking inside it - ADD THIS
  ...
  showVerificationForm: function (type, email) {
    ...
    this.sendVerificationEmail(email, this.verificationCode, type);
  },
  ...
  handleLogin: function (e) {
    ...
    if (user) {
      // FIX: Only require verification for NEW registrations, not login
  ...
  handleRegister: function (e) {
    ...
    this.pendingUser = {
      ...
      password: password,
      verified: false,
      id: "user_" + Date.now(), // Add ID for booking.js compatibility
    };
  ...
  handleResetPassword: function (e) {
    ...
    const userIndex = users.findIndex(
      (u) => u.email === this.resetPasswordEmail
    );
  ...
  completeRegistration: function () {
    ...
    localStorage.setItem("currentUser", JSON.stringify(this.pendingUser));

    // Update UI
    this.updateUIAfterLogin(this.currentUser);

    this.showSuccessMessage("Registration successful! Email verified.");
    this.hideAuthModal();
  ...
  async sendVerificationEmail(email, code, type) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showSuccessMessage("Code generated: " + code); // Fallback for demo
      }
    } catch (error) {
